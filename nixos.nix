  ...
  config = {
    systemd.services =
      let
        ...
        mkServicesForPersistentStoragePath = persistentStoragePath:
          foldl' recursiveUpdate { } (map (mkBindOrLinkFileService persistentStoragePath) cfg.${persistentStoragePath}.files);
      in
    ...
    system.activationScripts =
      let
        ...
        bindOrLinkFileScript = persistentStoragePath: file:
          let
            targetFile = escapeShellArg (concatPaths [ persistentStoragePath file ]);
            mountPoint = escapeShellArg file;
          in
        ...
        mkBindOrLinkFileScripts = persistentStoragePath:
          foldl' recursiveUpdate { } (map (bindOrLinkFileScript persistentStoragePath) cfg.${persistentStoragePath}.files);

        bindOrLinkFileScripts =
          foldl' recursiveUpdate { } (map mkBindOrLinkFileScripts persistentStoragePaths);
      in
      dirCreationScripts // bindOrLinkFileScripts;
